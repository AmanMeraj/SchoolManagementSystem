package com.school.schoolmanagement.Admin.Accounts.Expense;
...
public class ActivityAddExpense extends Utility implements ChartApiHelper.ChartFetchCallback {
    ...
    private void addExpense() {
        ...
        binding.addExpense.setEnabled(false);

        // Set the expense data
        addExpense.setDate(date);
        addExpense.setChartId(selectedExpenseChartId);
        addExpense.setAmount(Double.parseDouble(amount));
        addExpense.setPaymentMethod(paymentMethod);

        // Log the request data
        Log.d("AddExpense", "Sending expense request:");
        Log.d("AddExpense", "Date: " + date);
        Log.d("AddExpense", "Chart ID: " + selectedExpenseChartId);
        Log.d("AddExpense", "Amount: " + amount);
        Log.d("AddExpense", "Payment Method: " + paymentMethod);
        Log.d("AddExpense", "Auth Token: " + (authToken != null ? "Present" : "Null"));

        viewModel.addExpense(authToken, addExpense).observe(this, response -> {
            // Re-enable button first
            binding.addExpense.setEnabled(true);

            // Log the complete response
            Log.d("AddExpense", "Response received:");
            Log.d("AddExpense", "Success: " + response.isSuccess);
            Log.d("AddExpense", "Message: " + response.message);

            if (response.data != null) {
                Log.d("AddExpense", "Response Data: " + response.data.toString());
            } else {
                Log.d("AddExpense", "Response Data: null");
            }

            if (response.isSuccess) {
                Log.d("AddExpense", "Expense added successfully");
    ...
    public void onSuccess(AccountGet response) {
        if (response != null) {
            ...
            for (AccountGet.Datum chart : response.data) {
                if ("EXPENSE".equalsIgnoreCase(chart.getChartType())) {
